...
interface EditPhoneScreenProps {
  onBack: () => void;
  onViewMail?: () => void;
  onNavigateHome?: () => void;
  onFileTaxes?: () => void;
...
export function EditPhoneScreen({ 
  ...
  const [activeField, setActiveField] = useState<'phone' | 'alternatePhone' | null>(null);
  ...
  const handleKeyPress = (key: string) => {
    if (key === 'backspace') {
      ...
      } else if (activeField === 'alternatePhone') {
        setAlternatePhone(prev => prev.slice(0, -1));
      }
    } else {
      if (activeField === 'phone') {
        setPhone(prev => formatPhoneNumber(prev + key));
      } else if (activeField === 'alternatePhone') {
        setAlternatePhone(prev => formatPhoneNumber(prev + key));
      }
    }
  };
    ...
    <div className="h-full bg-[#f2f2f7] flex flex-col">
      ...
      <div className="flex-shrink-0 px-4 pt-2 pb-3 bg-[#f2f2f7] sticky top-[47px] z-10">
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-[28px] font-bold text-black m-0 tracking-tight">Edit phone numbers</h1>
        </div>
        <p className="text-black text-[15px] m-0 opacity-80 ml-[42.6px]">Update the phone number where the CRA can reach you</p>
      </div>
      ...
      >
        ...
        <div className="px-4 pb-3">
          <div className="card-ios p-4">
            <div className="mb-4">
              <Label htmlFor="phone" className="text-black text-[15px] block mb-2">
              ...
              <Input
                ...
                onFocus={() => { setShowKeyboard(true); setActiveField('phone'); }}
            ...
            <div className="mb-4">
              <Label htmlFor="alternatePhone" className="text-black text-[15px] block mb-2">
              ...
              <Input
                ...
                onChange={(e) => setAlternatePhone(formatPhoneNumber(e.target.value))}
                onFocus={() => { setShowKeyboard(true); setActiveField('alternatePhone'); }}
                className="w-full px-3 py-2 border border-[#c7c7cc] rounded-[8px] bg-white text-black text-[17px] focus:border-[#007AFF] focus:ring-1 focus:ring-[#007AFF]"
                placeholder="(555) 555-5555"
              />
            </div>

            <div className="bg-[#fff3cd] border-l-4 border-[#ffc107] p-3 rounded-[8px]">
              <p className="text-black text-[15px] m-0">
                <strong>Note:</strong> The CRA may use this number to contact you about your account or tax matters.
              </p>
            </div>
          </div>
        </div>
